...
class Bomb(Entity):
    def __init__(self, position, texture):
        super().__init__(position=position, model='quad', texture=texture, scale=(14, 14), collider='box')
        self.velocity = Vec2(uniform(-200,200), 20)
        self.bounce = 1.0

class Player(Entity):
    def __init__(self, position, texture):
        super().__init__(position=position, model='quad', texture=texture, scale=(32, 48), collider='box')
...
class PhaserPlatformerGame:
    def __init__(self):
        ...
        for i in range(12):
            x = 12 + i * 70
            star = Star(position=(x, 600), texture=self.star_texture)
            self.stars.append(star)

        self.bombs = []
        self.score_text = Text(text='Score: 0', position=(-0.48, 0.43), origin=(0, 0), scale=2)
        self.game_over_text = Text(text='GAME OVER', origin=(0, 0), scale=3, enabled=False)
    ...
    def move_player(self):
        ...
        if held_keys['up arrow'] and player.on_ground:
        ...
        player.x = clamp(player.x, half_width, GAME_WIDTH - half_width)
        old_y = player.y
        player.y += player.velocity.y * time.dt
        player.on_ground = False

        for platform in self.platforms:
            platform_top = (platform.y + platform.scale_y / 2)
            platform_left = (platform.x - platform.scale_x / 2)
            platform_right = (platform.x + platform.scale_x / 2)
            player_left = (player.x - player.scale_x / 2)
            player_right = (player.x + player.scale_x / 2)
            ...
            if (
                ...
                player.velocity.y = (abs(player.velocity.y) * player.bounce)
        ...
        for star in self.stars:
            ...
            for platform in self.platforms:
                ...
                if (
                    ...
                    star.velocity.y = (abs(star.velocity.y) * star.bounce_y)
        ...
        for bomb in self.bombs:
            ...
            for platform in self.platforms:
                ...
                if (
                    ...
                    bomb.y = (platform_top + bomb.scale_y / 2)
                    bomb.velocity.y = (abs(bomb.velocity.y) * bomb.bounce)
    ...
    def collect_star(self, star):
        ...
        if len(active_stars) == 0:
            ...
            bomb = Bomb(position=(x, 584), texture=self.bomb_texture)
    ...
    def check_star_collisions(self):
        for star in self.stars:
            if not star.active:
                continue
            if self.overlaps(self.player, star):
                self.collect_star(star)

    def check_bomb_collisions(self):
        for bomb in self.bombs:
            if self.overlaps(self.player, bomb):
                self.hit_bomb(bomb)
                return
