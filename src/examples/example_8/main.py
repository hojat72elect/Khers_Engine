...
for n in range(12):
    for k in range(12):
        box = Button(color=color.white, model="cube", position=(k, 0, n), texture="assets/grass", parent=scene, origin_y=0.5)
        boxes.append(box)
    ...
    for box in boxes:
        if box.hovered:
            if key == "left mouse down":
                new = Button(color=color.white, model="cube", position=box.position + mouse.normal, texture="assets/grass", parent=scene, origin_y=0.5)
                boxes.append(new)
