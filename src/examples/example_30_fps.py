...
def shoot():
    if not gun.on_cooldown:
        ...
        if mouse.hovered_entity:
            # Find the actual Enemy (might be hovering over health_bar child)
            ...
            if target and isinstance(target, Enemy):
                target.hp -= 10
                # Use color animation instead of blink() to avoid shader issues
                target.animate_color(color.red, duration=0.1)
                target.animate_color(color.light_gray, duration=0.1, delay=0.1)
