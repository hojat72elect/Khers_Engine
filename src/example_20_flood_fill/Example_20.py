...
def create_icons():
    ...
    for color_name, x, y in ICON_POSITIONS:
        ...
        # Start off screen just like Phaser.
        if x < 400:
            monster.x = px(-200)
            shadow.x = px(-200)
        else:
...
def create_cursor():
    ...
    cursor_indicator = create_sprite(
        "cursor-over",
        0,
        0,
        z=10,
...
class Particle(Entity):
    ...
    def __init__(self, x, y, texture_name):
        super().__init__(
            model="quad",
            texture=f"{texture_name}.png",
            position=(px(x), py(y), 20),
        )
        ...
        self.velocity = Vec3(
            py_random.uniform(-160, 160),
            py_random.uniform(-180, 80),
            0,
        )
    ...
    def update_particle(self):
        ...
        # Convert normal screen-space Y velocity to Ursina coordinates.
...
def icon_hovered(data):
    ...
    icon_color = COLORS.index(
        next(
            name for name, value in icons.items()
            if value is data
        )
    )
...
def update():
    ...
    if arrow.enabled:
        ...
        arrow.x = px(
            arrow_base_x
            + 12
            + 12 * __import__("math").sin(arrow_time * 3.5)
        )

    # Monster hover animation.
    ...
    if hovered_icon is not None and allow_click:
        ...
        if monster_bounce is not None:
            # Return monster to its normal position.
            for data in icons.values():
                if data["monster"] is monster_bounce:
...
# Do some initial floods to make the opening board easier.
help_flood()
